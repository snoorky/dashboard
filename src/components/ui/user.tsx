...
export function User() {
	...
	return (
		<div className="relative">
			...
			{showUserDropdown && (
				<div className="absolute top-full right-0 z-50 p-3 rounded-xl min-w-52 text-sm border border-[#e5e7eb] bg-white">
					<p className="lg:hidden font-medium">{company?.business_name}</p>
					<p className="font-light">{user?.email}</p>
					<button
						// onClick={handleLogout}
						className="w-full mt-3 flex items-center gap-2 pt-3 border-t border-[#e5e7eb] text-primary"
					>
