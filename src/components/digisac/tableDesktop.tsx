...
export function TableDesktop({ reportsByPeriod, selectedPeriod, selectedOperator, operatorOptions }: TableDesktopProps) {
  ...
  const data = useMemo(() => {
    const reports = reportsByPeriod[selectedPeriod] || [];
    const filteredReports = selectedOperator === "Todos" ? reports : reports.filter((r) => `${r.operator}||${r.department}` === selectedOperator);
    ...
    for (const report of filteredReports) {
      ...
      if (!grouped.has(key)) {
        grouped.set(key, { operator: report.operator, department: report.department, report });
      }
    }
    ...
    return Array.from(grouped.values()).map(({ operator, department, report }) => {
      ...
      return { operator, department, values };
    });
        ...
        <table className="min-w-max w-full">
          ...
          <tbody>
            {data.map(({ operator, department, values }, index) => {
              ...
              const isDimmed = selectedOperator !== "Todos" && selectedOperator !== key;
