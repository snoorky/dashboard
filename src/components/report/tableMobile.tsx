...
export function TableMobile({ data }: { data: Report[] }) {
  ...
  const processedData = useMemo(() => {
    ...
    return metricsTemplate.map(metric => {
      ...
      if (metric.type === "time") value = getAverageTime(data.map(report => report[metric.key as keyof Report] as string))
      else if (metric.type === "count") value = data.reduce((total, report) => total + Number(report[metric.key as keyof Report] || 0), 0).toString()

      return { ...metric, value }
    })
  }, [data, metricsTemplate])
